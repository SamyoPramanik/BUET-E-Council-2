...
2. **Retrieve** up to 5 precedents via `resolution_chunks`, **hard-filtered to the same `category_id`** (never widened to other categories even when results are sparse — a cross-category "precedent" would mislead more than help) and ranked by cosine similarity. A row's `decision_type` matching the one the author picked only affects ranking/confidence, since most historical rows won't have one set (see the migration note below).
3. **Generate — always**, regardless of how strong the top match is. A tiered design was considered and explicitly rejected: a shortcut that copies a near-duplicate precedent's resolution text verbatim (with only entity-slot substitution) outside the model can't notice when a superficially-similar agenda is actually substantively different — e.g. two agendas that are both "Dr. X, Associate Professor, CSE Dept, applied for N days of [leave type]" score very high textual similarity, but casual leave and study-leave-abroad need entirely different resolution clauses (bond, security, replacement-teacher arrangement), and a blind copy has no mechanism to notice that. Always routing through generation means the model always reads the full current agenda text and can catch that distinction itself. The cost: every autofill click always pays for one generation call (5-20s on CPU) even for cases that a tiered design could have answered instantly — accepted deliberately as the trade for removing that whole class of silent-wrong-answer risk, since this is a manual button click, not a hot loop.
4. **Confidence label is a function of retrieval quality, not of a code path** — `computeConfidence()`: `near_identical_precedent` (top match ≥95% similarity *and* matching decision type — conservative on purpose, since a high aggregate score can hide the fact that the one phrase that actually determines the correct resolution differs, drowned out by shared boilerplate around it; loosen only after observing real score distributions), `consistent_pattern` (≥3 precedents, ≥2 sharing the chosen decision type), `weak_precedent` (some precedents, pattern not tight), `unconfirmed` (no/sparse same-category precedents — still drafts something rather than refusing, per product decision, just clearly flagged).
5. The model is instructed (system prompt in `resolutionAutofill.js`) to use only facts present in the agenda/rough draft and emit an explicit `[placeholder]` for anything else a proper resolution needs but wasn't given — never to invent a specific name/date/amount. Placeholders are extracted (`extractPlaceholders`) and, in the returned HTML, wrapped in `<mark data-placeholder="true">` so they stay visually obvious in both the editor and any PDF export (`styleRichTextHtml` already fully styles `<mark>`).

**Decision-type dropdown**: `RESOLUTION_DECISION_TYPES` (approved / rejected / approved_with_conditions / deferred / referred_to_committee / amended_and_approved / noted / withdrawn) is a fixed list, not free-form inference — kept in two places by hand: `resolutionAutofill.js` (validation source of truth) and `ResolutionView.tsx` (Bangla/English labels for the same values). Selecting one and saving writes it to the new `agenda.decision_type` column (`db/migrations/2026_09_add_resolution_decision_type.sql`) via `COALESCE($new, decision_type)` in `createResolution`/`updateResolution`, so a plain manual save that doesn't touch the dropdown never clobbers a previously-set value. That column is **deliberately never backfilled** onto historical resolutions — there's no reliable way to infer a decision type from years of free-text without risking a wrong guess corrupting the same-category-same-decision ranking above; it simply starts filling in as new resolutions are authored through this flow.

**Frontend** (`ResolutionView.tsx`): the decision-type dropdown and an "Autofill AI" button sit above the resolution `RichTextEditor`, plus a **global** Direct Edit / Preview toggle (persisted to `localStorage`, applies to every autofill click on the page until changed — not a per-click prompt). Direct Edit replaces the editor content immediately with a basis panel underneath (confidence badge, which past resolutions it drew from, any remaining placeholders); Preview instead renders the draft in a dashed-border panel with explicit Accept/Discard buttons, and nothing touches the real resolution box until Accept. Either way, nothing auto-saves — the existing Save button still commits it, same as before this feature existed.

**Fine-tuning path (not yet done, sized for when there's enough real data)**: the off-the-shelf model above needs no training data. If/when fine-tuning on this institution's own history is worthwhile, size the decision by real volume — under ~200 real `(agenda, resolution)` examples, don't fine-tune (a LoRA run would overfit/memorize rather than generalize; rely on retrieval + prompting alone); 1000+ (this deployment's actual scale) is solidly enough. Train on the *varied* categories only — categories where resolutions are essentially "always the same wording depending on approved/rejected" don't need the model to learn a pattern the retrieval context already supplies verbatim every time, and including many near-duplicate examples would waste training capacity. Recipe: QLoRA via Unsloth (fits an 8B model in 16GB VRAM on a single consumer GPU), 2-3 epochs, format each historical row as `(category, decision_type, agenda content_plain) → resolution_plain` — not a synthetic rough-draft mapping, since real historical data has no rough-draft input; that part is handled at inference time by the retrieval prompt instead, and fine-tuning's job is teaching phrasing/formality/Bangla fluency. Merge the LoRA adapter, convert to GGUF via llama.cpp, quantize (Q4_K_M), and `ollama create` the result under the same `RESOLUTION_AI_MODEL` name — no architecture or code change needed to swap in the fine-tuned model.
...
| `POST` | `/api/agendas/:id/resolutions/autofill` | AI Resolution Autofill — body `{ roughDraft, decisionType }`, returns `{ html, confidence, placeholders, sources }`; nothing here writes to the DB, see [3.8 AI Resolution Autofill](#38-ai-resolution-autofill) |
