...
- [3. Deep Dive: Core Engineering Subsystems](#3-deep-dive-core-engineering-subsystems)
  ...
  - [3.8 AI Resolution Autofill](#38-ai-resolution-autofill)
...
**Historical bug (fixed 2026-09-17):** `@tiptap/extension-table`'s own `Table.renderHTML` (`createColGroup`/`getTableStyle` in the installed v3 package) unconditionally bakes its own `<colgroup>` *and* an absolute editor-canvas-px `width` (or `min-width`, if not every column was resized) into every table's saved HTML — that's what column-dragging and the Stretch-to-Page/Original-Size toggle actually persist as, entirely independent of this app's `data-width-mode`/`colwidth` logic above. Two compounding bugs followed: (1) `styleRichTextHtml` appended its own freshly-computed `<colgroup>` *in front of* that pre-existing one instead of replacing it, so every table shipped two `<colgroup>`s; (2) `mergeStyle` only fills in properties that aren't already present in the existing `style=` string, and a literal `style="width: 1113px"` already contains the substring `"width"`, so the function's own `width: 100%` / `width: auto` override was silently skipped and the stale editor-canvas pixel width (which can vastly exceed the printable page width) won. Net effect: resizing a column, or switching width mode, stopped reflecting in the PDF — exactly the scenario the 09-15 fix was meant to cover, just from a different source. Fixed by stripping any existing `<colgroup>` out of the table's inner HTML before appending this app's own, and by stripping any existing `width`/`min-width`/`max-width`/`table-layout` declarations out of the table's `style=` attribute before merging in the computed sizing, so this app's sizing always wins instead of being conditionally skipped.
...
### 3.8 AI Resolution Autofill

`POST /api/agendas/:id/resolutions/autofill` (implementation: [`meeting_service/controllers/agendaController.js`](file:///media/samyo-pramanik/New%20Volume2/buet-ecouncil2/meeting_service/controllers/agendaController.js)'s `autofillResolution`, prompt/HTML/confidence logic in [`meeting_service/utils/resolutionAutofill.js`](file:///media/samyo-pramanik/New%20Volume2/buet-ecouncil2/meeting_service/utils/resolutionAutofill.js)) drafts a formal resolution from a short rough decision the author types into the resolution box, using the institution's own past resolutions as precedent. Nothing here writes to the database — the caller reviews/edits the returned HTML and saves it through the existing `createResolution`/`updateResolution` endpoints exactly like manually-typed text. There is no decision-type input anywhere in this flow — the author just types the rough decision, nothing else.

**Model**: Aya Expanse 8B (Q4_K_M GGUF), served locally and for free via a plain upstream `ollama/ollama` container (new `resolution_ai` service in `docker-compose.yml`, behind the same opt-in `embeddings` Compose profile as `embedding_service`) — CPU-only at inference, no external API calls, no per-request cost. Aya Expanse was picked over the more commonly-used Llama 3.1/Qwen2.5 specifically because it's explicitly multilingual-trained including Bengali; this app's resolutions are heavily Bangla/English-mixed and those other models are English-first with weak Bangla generation. `meeting_service` talks to Ollama's own REST API directly (`POST http://resolution_ai:11434/api/generate`, `format: 'json'` so each turn's structured reply parses reliably) — no custom Python wrapper service, same pattern as `embedding_service`'s `/embed`. The image ships with no model baked in; `ollama pull aya-expanse:8b` (or `ollama create` for a fine-tuned Modelfile, see below) must be run once against the container before first use.

**Pipeline — retrieval, then a clarify-or-complete generation loop:**

1. **Embed** the current agenda's `content_plain` + the author's rough draft via the existing `embedding_service` (`embeddingClient.js`'s `embedTexts`, the same call `searchController.js` already makes for search) — if unreachable, generation proceeds with zero precedents rather than failing the request.
2. **Retrieve** up to 5 precedents via `resolution_chunks`, ranked by cosine similarity. Hard-filtered to the same `category_id` when the current agenda has one — never widened to other categories even when results are sparse, since a cross-category "precedent" would mislead more than help. When the agenda has no category at all, that filter can't apply (`category_id = NULL` matches nothing in SQL), so retrieval falls back to a similarity search across all categories instead of silently returning zero precedents forever.
3. **Generate or ask — always routes through the model**, regardless of how strong the top match is. A tiered design that short-circuits to a verbatim copy of a near-duplicate precedent was considered and explicitly rejected: it happens outside the model, so it can't notice when a superficially-similar agenda is actually substantively different — e.g. two agendas that are both "Dr. X, Associate Professor, CSE Dept, applied for N days of [leave type]" score very high textual similarity, but casual leave and study-leave-abroad need entirely different resolution clauses (bond, security, replacement-teacher arrangement), and a blind copy has no mechanism to notice that. Always routing through generation means the model always reads the full current agenda text and can catch that distinction itself.
4. **The model responds with one of two JSON shapes** (`runGenerationTurn` in `resolutionAutofill.js`): `{"status": "needs_clarification", "questions": [...]}` (1-3 questions, only for things that would genuinely change the wording — a missing amount, an unclear duration, whether a condition applies) or `{"status": "complete", "resolution": "..."}`. The model is instructed to never invent a specific name/date/amount/committee and never leave a `[placeholder]` — if something is missing, it asks instead. `MAX_CLARIFICATION_ROUNDS` (4) is a safety valve: once the conversation hits that many answered rounds, the prompt forces `status: "complete"` using best judgment rather than asking again. A defensive `[placeholder]` extraction/highlighting still runs on the final text (`extractPlaceholders`, wrapped in `<mark data-placeholder="true">` in the returned HTML) in case the model leaks one through anyway despite the instruction.
5. **Confidence label is a function of retrieval quality alone** — `computeConfidence()`: `near_identical_precedent` (top match ≥95% similarity), `consistent_pattern` (≥3 precedents found), `weak_precedent` (some precedents, pattern not tight), `unconfirmed` (no/sparse same-category precedents — still drafts something rather than refusing, per product decision, just clearly flagged). There is no decision-type signal feeding this — the model reads each precedent's actual resolution text directly instead of needing a separate structured label (a fixed decision-type dropdown/column was tried and dropped; see git history for `db/migrations/2026_09_add_resolution_decision_type.sql`).

**The clarifying-question loop** (`agendaController.js`'s `autofillResolution`, request body `{ roughDraft, conversation }`): each call is one round. The caller passes the growing `conversation` array (`{question, answer}` pairs so far); a response with `needsClarification: true` and `questions` means the frontend should collect answers and call again with those appended to `conversation`, repeating until a response comes back with `needsClarification: false` and the finished `html`/`confidence`/`placeholders`/`sources`.

**Frontend** (`ResolutionView.tsx`): an "Autofill AI" button sits above the resolution `RichTextEditor` — no decision-type input, just the rough decision typed into the resolution box — plus a **global** Direct Edit / Preview toggle (persisted to `localStorage`, applies to every autofill click on the page until changed — not a per-click prompt). If the model needs clarification, a floating chat widget (bottom-right FAB, slides out a side panel) shows the pending questions with a textarea per question and a "Skip / I don't know" shortcut per question; submitting answers re-calls the autofill endpoint with the accumulated conversation. Once complete, Direct Edit replaces the editor content immediately with a basis panel underneath (confidence badge, which past resolutions it drew from, any remaining placeholders); Preview instead renders the draft in a dashed-border panel with explicit Accept/Discard buttons, and nothing touches the real resolution box until Accept. Either way, nothing auto-saves — the existing Save button still commits it, same as before this feature existed.

**Fine-tuning path (not yet done, sized for when there's enough real data)**: the off-the-shelf model above needs no training data. If/when fine-tuning on this institution's own history is worthwhile, size the decision by real volume — under ~200 real `(agenda, resolution)` examples, don't fine-tune (a LoRA run would overfit/memorize rather than generalize; rely on retrieval + prompting alone); 1000+ (this deployment's actual scale) is solidly enough. Train on the *varied* categories only — categories where resolutions are essentially formulaic don't need the model to learn a pattern the retrieval context already supplies verbatim every time, and including many near-duplicate examples would waste training capacity. Recipe: QLoRA via Unsloth (fits an 8B model in 16GB VRAM on a single consumer GPU), 2-3 epochs, format each historical row as `(category, agenda content_plain) → resolution_plain` — not a synthetic rough-draft mapping, since real historical data has no rough-draft input; that part is handled at inference time by the retrieval prompt instead, and fine-tuning's job is teaching phrasing/formality/Bangla fluency. Merge the LoRA adapter, convert to GGUF via llama.cpp, quantize (Q4_K_M), and `ollama create` the result under the same `RESOLUTION_AI_MODEL` name — no architecture or code change needed to swap in the fine-tuned model.
...
| `GET` | `/api/meetings/:id/pdf/:type` | Download rendered PDF (`agenda`, `suppli-agenda`, `resolution`, `resolution-status`, `attendance`). Optional page-layout query params (`pageSize`, `orientation`, `marginTop/Right/Bottom/Left`, `scale`, `lineHeight`, `agendaNumberStyle`) from the PDF Preview page — see §3.4. `GET /api/meetings/:id/pdf/resolution?agenda_id=X` returns a single agenda+resolution pair document (university header 20px, body 14px, no tags). UI: per-card Preview (modal) + Download buttons in `ResolutionView`; the Materials “Separate Page per Resolution” button opens a selector popup (Bangla serial + pretext checkboxes, Select All toggle, PDF/Word format, Download → Download All label) that downloads one file per selected agenda |
| `GET` | `/api/meetings/:id/docx/resolution?agenda_id=X` | Download a single agenda+resolution pair as Word (.docx) |
...
| `POST` | `/api/agendas/:id/resolutions/autofill` | AI Resolution Autofill — body `{ roughDraft, conversation }`, returns `{ needsClarification, questions }` (ask another round) or `{ needsClarification: false, html, confidence, placeholders, sources }`; nothing here writes to the DB, see [3.8 AI Resolution Autofill](#38-ai-resolution-autofill) |
